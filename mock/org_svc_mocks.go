...
//
//	mockgen -destination=./org_svc_mocks.go -package=mock github.com/calculi-corp/api/go/auth OrganizationsServiceClient
//
...
func (m *MockOrganizationsServiceClient) MarkOrganizationInternal(ctx context.Context, in *auth.UpdateOrganizationRequest, opts ...grpc.CallOption) (*auth.UpdateOrganizationResponse, error) {
	m.ctrl.T.Helper()
	varargs := []any{ctx, in}
	for _, a := range opts {
		varargs = append(varargs, a)
...
func (mr *MockOrganizationsServiceClientMockRecorder) MarkOrganizationInternal(ctx, in any, opts ...any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	varargs := append([]any{ctx, in}, opts...)
